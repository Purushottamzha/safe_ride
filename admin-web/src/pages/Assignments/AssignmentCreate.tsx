...
import { Box, Grid, Card, CardContent, TextField, MenuItem, Alert } from '@mui/material';
...
export default function AssignmentCreate() {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<AssignmentForm>({
    ...
    defaultValues: { schoolId: isSuperAdmin ? '' : user?.schoolId || '', name: '', routeId: '' },
  });

  const createMutation = useMutation({
    mutationFn: (data: AssignmentForm) => assignmentService.create({ ...data, isActive: true }),
  ...
  return (
    <Box>
      <PageHeader title="Create Assignment" subtitle="Assign route to a school" showBack backTo="/assignments" />
      <Grid container spacing={3}>
        <Grid item xs={12} md={8} lg={6}>
          <Card>
            <CardContent sx={{ p: 3 }}>
              {createMutation.isError && <Alert severity="error" sx={{ mb: 2 }}>Failed to create assignment</Alert>}
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12}>
                    <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                      <LoadingButton variant="contained" type="submit" loading={createMutation.isPending}>Create Assignment</LoadingButton>
                    </Box>
                  </Grid>
                </Grid>
              </Box>
