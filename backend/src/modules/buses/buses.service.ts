...
export class BusesService {
  ...
  async findAll(params: { page?: number; limit?: number; search?: string; schoolId?: string; status?: BusStatus }) {
  ...
  async update(id: string, data: Partial<{
    plateNumber: string; busNumber: string; model: string; capacity: number;
    year: number; color: string; status: BusStatus; gpsDeviceId: string;
    cameraDeviceId: string; lastGpsLat: number; lastGpsLng: number; lastGpsUpdate: string;
  }>) {
  ...
  async softDelete(id: string): Promise<void> {
    const bus = await this.prisma.bus.findFirst({ where: { id, deletedAt: null } });
    if (!bus) throw new NotFoundException('Bus not found');
    await this.prisma.bus.update({ where: { id }, data: { deletedAt: new Date(), status: 'INACTIVE' } });
  }
}
